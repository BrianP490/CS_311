...
//======================================================
// HW#: HW6 dgraph
// Name: Brian Perez   ****Not current file***
...
void dgraph::fillTable()  // be sure to read from a specified file
{
  ...
  int temp;  //used when looping in the for loop	
  
  while(fin >> Gtable[countUsed].vertexName)   //loop until the function reads the whole text file
    {
     	fin >> line;  //might need line to be an int
	Gtable[countUsed].outDegree =line - 48;  //might replace with fin >>Gtable[countUsed].outDegree  
